...
def outputStats_assess(KAPPA_AE, OA_AE, AA_AE, ELEMENT_ACC_AE, TESTING_TIME_AE, CATEGORY, path1, path2):
    ...
    sentence2 = 'AAs, mean_AA ± std_AA for each iteration are:' + str(AA_AE) + str(np.mean(AA_AE)) + ' ± ' + str(np.std(AA_AE)) + '\n'
    f.write(sentence2)
    sentence3 = 'Total average Testing time is:' + str(np.sum(TESTING_TIME_AE)) + '\n'
    f.write(sentence3)
